...
function MobileNavigation({
  ...
  return <div className="p-3 space-y-1">
      {sidebarLinks.map(link => <Button key={link.path} variant="ghost" className={`w-full justify-start ${isActivePath(link.path) ? 'bg-amber-200/50 dark:bg-amber-900/30 text-amber-900 dark:text-amber-300' : 'hover:bg-amber-100/50 dark:hover:bg-amber-900/20 text-amber-800 dark:text-amber-400'}`} onClick={() => handleNavigation(link.path)}>
      ...
      <AnimatePresence mode="wait">
        ...
        duration: 0.3
            ...
            <div className="flex justify-between p-4">
              ...
              <motion.button onClick={toggle} whileHover={{
            scale: 1.1
          }} whileTap={{
            scale: 0.95
            ...
            <SidebarContent className="p-2">
              <nav>
                {sidebarLinks.map((link, index) => <motion.div key={link.path} initial={{
              x: -20,
              opacity: 0
            }} animate={{
              x: 0,
            ...
            }} className={`flex items-center gap-3 px-3 py-2 rounded-md transition-colors mb-1 cursor-pointer ${isActivePath(link.path) ? "bg-amber-300/40 dark:bg-amber-900/30 text-amber-900 dark:text-amber-200" : "hover:bg-amber-200/30 dark:hover:bg-amber-900/20 text-amber-700 dark:text-amber-400 hover:text-amber-900 dark:hover:text-amber-200"}`} onClick={() => handleNavigation(link.path)}>
