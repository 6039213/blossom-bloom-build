...
import { Send, Smartphone, Tablet, Monitor, Copy, Download, Code, Eye, FileText } from 'lucide-react';
...
export default function BlossomsAIWebBuilder() {
  ...
  const handleSubmitPrompt = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save the raw response for advanced view
...
The code will be directly executed in a preview environment, so it must be complete and error-free.`;
      ...
      const response = await fetch('/api/claude', {
        ...
        body: JSON.stringify({
          system: systemPrompt,
          prompt: `Create a complete website based on this description: "${userPrompt}". Provide all necessary files to make it functional.`,
          temperature: 0.7,
          max_tokens: 4000
        })
      });
  ...
  const handleDownloadProject = () => {
    ...
    toast.info("Downloading project...");
    
    // In a real implementation we'd use JSZip to create a zip file
    setTimeout(() => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
      ...
      <div className="lg:col-span-1 flex flex-col bg-white dark:bg-gray-900 rounded-xl shadow-lg border border-border overflow-hidden">
      ...
      <div className="lg:col-span-2 flex flex-col bg-white dark:bg-gray-900 rounded-xl shadow-lg border border-border overflow-hidden">
